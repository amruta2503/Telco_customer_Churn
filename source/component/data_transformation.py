...
class DataTransformation:
    ...
    def min_max_scaling(self, data, key=None):
        ...
        if key == 'predict':
            ...
            # 3. Check data shape
            print(data.shape)  # Ensure it's (1000, 19)

            # 4. Scale the data using Min-Max Scaler
            scaler_details = pd.read_csv('source/ml/scaler_details.csv')

            for col in data.columns:
                if col in scaler_details['Feature'].values:
                    min_val = scaler_details.loc[scaler_details['Feature'] == col, 'Scaler_Min'].values[0]
                    max_val = scaler_details.loc[scaler_details['Feature'] == col, 'Scaler_Max'].values[0]
    ...
    def oversample_smote(self, data):
        try:
            ...
            imputer = SimpleImputer(strategy='mean')  # You can choose strategy='median' or 'most_frequent' based on your data
    ...
    def initiate_data_transformation(self, key):
        ...
        if key == 'predict':
            ...
            #predict_data = predict_data.iloc[:-2]
